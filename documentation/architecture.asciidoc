...
We reflect this architecture in our code as described in our link:coding-conventions.asciidoc_packages[coding conventions] allowing a traceability of business components, use-cases, layers, etc. into the code and giving
...
This is illustrated by the horizontal connectors. Please note that link:guide-jpa.asciidoc_entity[persistence entities] are part of the API of the data-access component part so only the logic component part of the same
